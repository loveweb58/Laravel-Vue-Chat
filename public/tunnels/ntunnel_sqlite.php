    ...
    if (is_file($file)) {
        ...
        if ($fhandle) {
            ...
            if (strncmp($string, $sqlite2header, strlen($sqlite2header)) == 0) {
                ...
                if ($string == "" || strncmp(substr($string, 0, strlen($sqlite3header)), $sqlite3header, strlen($sqlite3header)) == 0) {
...
function doSystemTest()
{
    ...
    output("[SQLite2] PHP version >= 5.0.0", phpversion_int() >= 50000, ["Yes", "No"]);
    output("[SQLite2] sqlite_open() available", function_exists("sqlite_open"), ["Yes", "No"]);
    output("[SQLite3] PHP version >= 5.3.0", phpversion_int() >= 50300, ["Yes", "No"]);
    output("[SQLite3] SQLite3 class available", class_exists("SQLite3"), ["Yes", "No"]);
