...
function show_sidebar_menu(with_animation) {
    ...
    if (!with_animation) {
        ...
        public_vars.$sidebarMenu.add($sidebar_inner)
                   .transit({width: expanded_width}, public_vars.sidebarTransitionTime / 2);
...
function toggle_sidebar_menu(with_animation) {
    ...
    if (open) {
        $.post(Config.defaultURL + "/settings/change-sidebar-menu-state");
        show_sidebar_menu(with_animation);
        ps_init();
    }
    else {
        $.post(Config.defaultURL + "/settings/change-sidebar-menu-state", {state: 'sidebar-collapsed'});
