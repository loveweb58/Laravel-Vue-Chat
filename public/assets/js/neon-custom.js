                    ...
                    tile_stats.fullyEnterViewport(function () {
                        ...
                        TweenLite.to(o, duration / 1000, {
                            curr: end, ease: Power1.easeInOut, delay: delay / 1000, onUpdate: function () {
                                if (o.curr === end) {
                                    $num.html(prefix + (format ? numberWithCommas(o.curr) : o.curr) + postfix);
                                } else {
...
function setCurrentProgressTab($rootwizard, $nav, $tab, $progress, index) {
    ...
    if ($first_tab.hasClass('active')) {
        ...
        if (rtl()) {
            $progress.width($progress.parent().outerWidth(true) - $tab.prev().position().left - $tab.find('span')
                                                                                                    .width() / 2);
        }
        else {
    ...
    $(".checkbox-replace:not(.neon-cb-replacement), .radio-replace:not(.neon-cb-replacement)").each(function (i, el) {
        ...
        $input.on('change', function (ev) {
            if (is_radio) {
                ...
                $(".neon-cb-replacement input[type=radio][name='" + name + "']:not(:checked)")
